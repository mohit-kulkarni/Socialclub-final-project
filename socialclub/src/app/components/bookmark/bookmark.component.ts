...
export class BookmarkComponent implements OnInit {
  ...
  constructor(private bookmarkService: BookmarkService, private postservice: PostService, private http: HttpClient) { }
  ngOnInit(): void {
  ...
  loadFavoriteMovies(userId: number): void {
    this.bookmarkService.getUserBookmarks(userId).subscribe({
      next: (bookmarks) => {
        const postDetailsRequests: Observable<any>[] = bookmarks.map(bookmark => 
          this.postservice.getPostById(bookmark.post));
        ...
        forkJoin(postDetailsRequests).subscribe({
          next: (postsDetails) => {
            ...
            this.bookmarkedPostsDetails.forEach(postDetail => {
              ...
              postDetail.mediaType = this.getMediaType(postDetail.image_or_video);
